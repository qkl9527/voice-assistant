...
class OpenRouterService(BaseLLMService):
    ...
    def check_availability(self) -> bool:
        ...
        try:
            ...
            logger.error(f"OpenRouter服务不可用: {e}")
            return False
    ...
    async def generate_text(self, prompt: str, **kwargs) -> str:
        ...
        """
        if not self.is_available:
            logger.error("OpenRouter服务不可用")
            return "服务不可用，请检查API配置"
            
        try:
            # 获取模型参数
            model = kwargs.get("model") or self.config.get("model") or "openai/gpt-3.5-turbo"
            ...
            headers = {
                ...
                "HTTP-Referer": "https://voice-assistant.app",  # OpenRouter需要提供来源
                "X-Title": "Voice Assistant"  # 应用名称
            }
            ...
            if response.status_code == 200:
                ...
                if "choices" in result and len(result["choices"]) > 0:
                    ...
                    logger.error(f"OpenRouter响应格式异常: {result}")
                    return "响应格式异常"
            else:
                logger.error(f"OpenRouter请求失败: {response.status_code}, {response.text}")
