...
class AWSService(BaseLLMService):
    ...
    async def generate_text(self, prompt: str, **kwargs) -> str:
        ...
        try:
            # 获取模型参数
            model = kwargs.get("model") or self.config.get("model") or "anthropic.claude-v2"
            ...
            if response.status_code == 200:
                ...
                if "anthropic" in model:
                    ...
                    return result.get("completions", [{}])[0].get("data", {}).get("text", "")
                elif "cohere" in model:
                    return result.get("generations", [{}])[0].get("text", "")
                else:
                    return str(result)
            else:
                logger.error(f"AWS Bedrock请求失败: {response.status_code}, {response.text}")
