        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "choices" in result and len(result["choices"]) > 0:
                    ...
                    logger.error(f"LiteLLM响应格式异常: {result}")
                    return "响应格式异常"
            else:
                logger.error(f"LiteLLM请求失败: {response.status_code}, {response.text}")
                return f"请求失败: {response.text}"
        except Exception as e:
            logger.error(f"LiteLLM文本生成失败: {e}")
